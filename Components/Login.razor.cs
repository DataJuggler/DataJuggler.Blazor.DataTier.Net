        ...
        #region Methods
            ...
            public async Task<bool> HandleRememberPassword()
            {
                ...
                if (HasParentIndexPage)
                {
                    ...
                    if ((RememberLogin) && (HasLoggedInMember))
                    {
                        // Store the local store items
                        await ParentIndexPage.StoreLocalStoreItems(LoggedInMember.EmailAddress, LoggedInMember.PasswordHash);                        
                    }
                    else
                    {
            ...
            #region ProcessNewUserSignUp(object memberObject)
            /// <summary>
            /// This method Process New User Sign Up
            /// </summary>
            public async void ProcessVerifyPassword(object memberObject)
            {
                try
                {
                    // cast the object as a Member object
                    ...
                    if (NullHelper.Exists(member))
                    {
                        ...
                        bool verified = CryptographyHelper.VerifyHash(password, keyCode, member.PasswordHash);
                        ...
                        {
                             // Set the LoggedInMember on this page
                            LoggedInMember = member;

                            // Set the LoggedInUser
                            ParentIndexPage.LoggedInMember = member;
            ...
            public async Task<bool> StartProcessLogin()
            {
                ...
                {
                    // Attempt to find the user
                    member = await MemberService.FindMemberByEmailAddress(EmailAddress);
                }
                else
                {
                    // Show a message
